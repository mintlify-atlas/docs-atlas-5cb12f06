...
sidebarTitle: "Layers"
description: "Each of Forge's four architectural layers has strict ownership rules. Learn what belongs in Platform, Features, Shared, and Infrastructure and why the separation matters."
---
...
Platform must never hold domain entities, use cases, business logic, or any artifact with the suffixes `.entity.ts`, `.uc.ts`, `.mapper.ts`, or `.port.ts`. It must not import from `src/features/`.
