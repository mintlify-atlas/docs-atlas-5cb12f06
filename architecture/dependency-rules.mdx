...
description: "Forge enforces 13 architectural dependency rules (R1–R13) with CRITICAL, ERROR, and WARNING severity. Learn which imports are permitted, which are prohibited, and how violations are detected."
---
...
| **R13** | Domain artifacts in `platform/` — A file inside `src/platform/` has a domain artifact suffix (`.entity.ts`, `.uc.ts`, `.mapper.ts`, `.port.ts`) or imports from `src/features/`. Platform is exclusively technical backbone. | CRITICAL |
...
## Import Convention Rules (R10–R12) in Practice

Rules R10–R12 apply to every file Forge generates and every file it audits. They exist because Node.js ESM with `moduleResolution: "nodenext"` requires explicit, fully-resolved import paths.
